...
public class FutFeverApp extends JFrame {
    ...
    private static class Game { String home, away, date, time, stadium; Game(String home,String away,String date,String time,String stadium){this.home=home;this.away=away;this.date=date;this.time=time;this.stadium=stadium;} }
    private final java.util.List<Game> GAMES = new ArrayList<>();
    // Informação básica das equipas
    private static class TeamInfo { String coach; java.util.List<String> players; TeamInfo(String coach, java.util.List<String> players){ this.coach = coach; this.players = players; } }
    ...
    private void initTeams(){
        ...
        TEAM_INFO.put("Portugal", new TeamInfo("Fernando Santos", java.util.Arrays.asList("Ronaldo","Bernardo Silva","Bruno Fernandes","Diogo Costa")));
        TEAM_INFO.put("Brasil", new TeamInfo("Tite", java.util.Arrays.asList("Neymar","Vinícius Júnior","Alisson","Casemiro")));
        TEAM_INFO.put("Argentina", new TeamInfo("Lionel Scaloni", java.util.Arrays.asList("Messi","Di María","Dybala","Emiliano Martínez")));
        TEAM_INFO.put("França", new TeamInfo("Didier Deschamps", java.util.Arrays.asList("Mbappé","Griezmann","Kanté","Lloris")));
        TEAM_INFO.put("Espanha", new TeamInfo("Luis Enrique", java.util.Arrays.asList("Morata","Pedri","Gavi","De Gea")));
        TEAM_INFO.put("Itália", new TeamInfo("Mancini", java.util.Arrays.asList("Immobile","Chiesa","Jorginho","Donnarumma")));
    }

    private void initGames(){
        // Preencher alguns jogos de exemplo com estádio associado
        GAMES.add(new Game("Portugal","Brasil","25 Jun","20:30","Estádio de Lusail"));
        GAMES.add(new Game("Argentina","França","26 Jun","18:00","Estádio Al Bayt"));
        GAMES.add(new Game("Espanha","Itália","27 Jun","21:00","Arena Norte"));
        GAMES.add(new Game("Portugal","Argentina","28 Jun","19:00","Estádio de Lusail"));
    }
    ...
    private JPanel userHome(CardLayout userCards, JPanel content) {
        ...
        addQuick(quick,"📅","Calendário", userCards, content, "jogos"); addQuick(quick,"🏟️","Estádios", userCards, content, "stadios"); addQuick(quick,"🪪","ID Digital", userCards, content, "perfil");
    ...
    private JPanel adminShell() {
        ...
        sidebar.add(brand); sidebar.add(label("ADMIN", 10, Font.BOLD, new Color(215,242,231))); sidebar.add(Box.createVerticalStrut(20));
    ...
    private JPanel stadiumsScreen(CardLayout userCards, JPanel content){
        JPanel p=new JPanel(); p.setBackground(Color.WHITE); p.setBorder(new EmptyBorder(24,22,10,22)); p.setLayout(new BoxLayout(p,BoxLayout.Y_AXIS)); p.add(label("🏟️  Estádios com jogos",24,Font.BOLD,GREEN)); p.add(Box.createVerticalStrut(16));
        ...
        for(String s: stadiums){ JPanel c=card(); c.setMaximumSize(new Dimension(430,76)); c.setLayout(new BorderLayout()); JLabel l = label(s,15,Font.BOLD,Color.BLACK); c.add(l, BorderLayout.CENTER);
            JButton ver = whiteButton("Ver jogos");
            ver.addActionListener(e -> {
                String key = "stadium:"+s;
                // se ainda não existe, adicionar painel com os jogos
                for(Component comp: content.getComponents()){
                    if (key.equals(comp.getName())) { userCards.show(content,key); return; }
                }
            ...
            p.add(c); p.add(Box.createVerticalStrut(10));
        }
        return p;
    }
    ...
    private JPanel stadiumGamesPanel(String stadium, CardLayout userCards, JPanel content){
        JPanel p=new JPanel(); p.setBackground(Color.WHITE); p.setBorder(new EmptyBorder(24,22,10,22)); p.setLayout(new BoxLayout(p,BoxLayout.Y_AXIS));
        p.add(label("Jogos em " + stadium,20,Font.BOLD,Color.BLACK)); p.add(Box.createVerticalStrut(12));
        boolean found=false;
        for(Game g: GAMES){ if (stadium.equals(g.stadium)){
            JPanel c = card(); c.setMaximumSize(new Dimension(430,80)); c.setLayout(new BorderLayout());
            ...
            p.add(c); p.add(Box.createVerticalStrut(10)); found=true;
        }}
        if (!found){ p.add(label("Nenhum jogo encontrado neste estádio.",14,Font.PLAIN,MUTED)); }
        JButton back = button("Voltar"); back.addActionListener(e -> { userCards.show(content, "stadios"); }); p.add(Box.createVerticalStrut(12)); p.add(back);
        return p;
    }
    ...
    private JPanel gamesScreen(CardLayout userCards, JPanel content){
        JPanel p=new JPanel(); p.setBackground(Color.WHITE); p.setBorder(new EmptyBorder(24,22,10,22)); p.setLayout(new BoxLayout(p,BoxLayout.Y_AXIS));
        p.add(label("⚽  Jogos",24,Font.BOLD,GREEN)); p.add(Box.createVerticalStrut(16));
        for(int i=0;i<GAMES.size();i++){
            ...
            det.addActionListener(e -> {
                ...
                JPanel details = gameDetailsPanel(GAMES.get(idx), userCards, content);
                details.setName(key);
                content.add(details, key);
                userCards.show(content, key);
            });
    ...
    private JPanel gameDetailsPanel(Game g, CardLayout userCards, JPanel content){
        JPanel p = new JPanel(); p.setBackground(Color.WHITE); p.setBorder(new EmptyBorder(24,22,10,22)); p.setLayout(new BoxLayout(p,BoxLayout.Y_AXIS));
        p.add(label(g.home + "  vs  " + g.away,22,Font.BOLD,GREEN)); p.add(Box.createVerticalStrut(8));
        p.add(label(g.date + "  " + g.time,14,Font.PLAIN,Color.BLACK)); p.add(label("Estádio: " + g.stadium,14,Font.PLAIN,MUTED)); p.add(Box.createVerticalStrut(12));
        ...
        buy.addActionListener(e -> {
            ...
            gbc.insets = new Insets(6,6,6,6); gbc.anchor = GridBagConstraints.WEST; gbc.fill = GridBagConstraints.HORIZONTAL; gbc.gridx = 0; gbc.gridy = 0;
            form.add(new JLabel("Telefone:"), gbc);
            gbc.gridx = 1; JTextField phoneField = new JTextField(12); form.add(phoneField, gbc);
            gbc.gridx = 0; gbc.gridy++; form.add(new JLabel("Nº Cartão Cidadão:"), gbc);
            gbc.gridx = 1; JTextField ccField = new JTextField(12); form.add(ccField, gbc);
            gbc.gridx = 0; gbc.gridy++; form.add(new JLabel("Quantidade:"), gbc);
            gbc.gridx = 1; JSpinner qty = new JSpinner(new SpinnerNumberModel(1,1,20,1)); form.add(qty, gbc);
            ...
            if (res == JOptionPane.OK_OPTION) {
                ...
                if (confirm == JOptionPane.YES_OPTION) {
                    // Aqui poderia guardar a compra; por agora mostrar confirmação
                    JOptionPane.showMessageDialog(this, String.format("Compra efetuada!\n%s vs %s\nQuantidade: %d\nTotal: %d€\nContacto: %s\nCC: %s", g.home, g.away, quantity, total, phone, cc));
                }
            }
        });
        p.add(buy);
        JButton back = whiteButton("Voltar aos jogos"); back.addActionListener(e -> userCards.show(content, "jogos")); p.add(Box.createVerticalStrut(12)); p.add(back);
        return p;
    }
    ...
    private JPanel teamsScreen(CardLayout userCards, JPanel content){
        JPanel p=new JPanel(); p.setBackground(Color.WHITE); p.setBorder(new EmptyBorder(24,22,10,22)); p.setLayout(new BoxLayout(p,BoxLayout.Y_AXIS));
        p.add(label("👕  Equipas",24,Font.BOLD,GREEN)); p.add(Box.createVerticalStrut(16));
        // obter equipas únicas preservando ordem
        java.util.Set<String> teams = new java.util.LinkedHashSet<>();
        for(Game g: GAMES){ teams.add(g.home); teams.add(g.away); }
        for(String t: teams){ JPanel c=card(); c.setMaximumSize(new Dimension(430,76)); c.setLayout(new BorderLayout()); JLabel l = label(t,15,Font.BOLD,Color.BLACK); c.add(l, BorderLayout.CENTER);
            JPanel actions = new JPanel(new GridLayout(2,1,0,6)); actions.setOpaque(false);
            ...
            actions.add(details); actions.add(ver);
    ...
    private JPanel teamGamesPanel(String team, CardLayout userCards, JPanel content){
        JPanel p=new JPanel(); p.setBackground(Color.WHITE); p.setBorder(new EmptyBorder(24,22,10,22)); p.setLayout(new BoxLayout(p,BoxLayout.Y_AXIS));
        p.add(label("Jogos de " + team,20,Font.BOLD,Color.BLACK)); p.add(Box.createVerticalStrut(12));
        ...
        p.add(detailsTeam); p.add(Box.createVerticalStrut(8));
        boolean found=false;
        for(int i=0;i<GAMES.size();i++){
            Game g = GAMES.get(i);
        ...
        JButton back = button("Voltar"); back.addActionListener(e -> { userCards.show(content, "equipas"); }); p.add(Box.createVerticalStrut(12)); p.add(back);
        return p;
    }

    // Painel que mostra detalhes da equipa: treinador e jogadores
    private JPanel teamDetailsPanel(String team, CardLayout userCards, JPanel content){
        JPanel p=new JPanel(); p.setBackground(Color.WHITE); p.setBorder(new EmptyBorder(24,22,10,22)); p.setLayout(new BoxLayout(p,BoxLayout.Y_AXIS));
        p.add(label("Detalhes da equipa: " + team,20,Font.BOLD,Color.BLACK)); p.add(Box.createVerticalStrut(12));
        TeamInfo info = TEAM_INFO.get(team);
        if (info != null) {
            p.add(label("Treinador: " + info.coach,14,Font.PLAIN,Color.BLACK)); p.add(Box.createVerticalStrut(8));
            p.add(label("Jogadores:",14,Font.BOLD,Color.BLACK));
            for(String pl: info.players){ p.add(label("- " + pl,13,Font.PLAIN,MUTED)); }
        } else {
            p.add(label("Sem informações de equipa.",14,Font.PLAIN,MUTED));
        }
        p.add(Box.createVerticalStrut(12));
        JButton back = button("Voltar às equipas"); back.addActionListener(e -> userCards.show(content, "equipas")); p.add(back);
        return p;
    }
