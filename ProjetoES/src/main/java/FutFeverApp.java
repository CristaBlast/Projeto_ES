...
public class FutFeverApp extends JFrame {
    ...
    private JComponent adminCalendario() {
        ...
        p.add(formCard()); p.add(Box.createVerticalStrut(18));
        SimpleDateFormat dateFmt = new SimpleDateFormat("dd MMM", Locale.of("pt","PT"));
        SimpleDateFormat timeFmt = new SimpleDateFormat("HH:mm");
        java.util.List<String[]> rows = new ArrayList<>();
        for (Jogo j : GestorDados.getInstance().getJogos()) {
    ...
    private JPanel formCard() {
        JPanel form = card(); form.setLayout(new GridLayout(2,3,12,12));
        form.add(input("Nome / título")); form.add(input("Data / país")); form.add(input("Estado"));
        form.add(input("Observações")); form.add(button("Guardar")); form.add(button("Limpar"));
        return form;
    }
    ...
    private JPanel bilhetesScreen() {
        ...
        if (bilhetes.isEmpty()) {
            ...
            for (Bilhete b : bilhetes) {
                ...
                JPanel c = card(); c.setMaximumSize(new Dimension(390,76)); c.setLayout(new BorderLayout());
    ...
    private JPanel profileScreen(){
        ...
        JButton back = button("Voltar ao início"); back.addActionListener(e -> cards.show(root,"index")); p.add(back);
        return p;
    }
